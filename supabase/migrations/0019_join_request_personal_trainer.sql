-- 0019_join_request_personal_trainer.sql
-- Let prospects optionally add a Personal Trainer plan when self-registering via
-- the public QR join form. The PT choice is snapshotted on the join request
-- (name + price) alongside the membership, and on approval it becomes a second,
-- independent subscription. The recorded payment is the combined total the
-- prospect actually paid (membership + PT) in one transaction.

alter table public.join_requests
  add column pt_plan_id    uuid references public.membership_plans(id) on delete set null,
  add column pt_plan_name  text,
  add column pt_plan_price numeric(10,2);
